...
public class RecipeCommandController {
	...
	public ResponseEntity<ApiResponse<RecipeRecommendResponse>> recommendRecipe(
		...
		RecipeRecommendResponse response = recipeCommandService.getRecipeRecommendation(request,
	...
	/**
	 * 추천 레시피를 내 레시피로 등록
	 * POST /api/v1/recipes/from-recommend/{rcdId}
	 */
	@PostMapping("/from-recommend/{rcdId}")
	public ResponseEntity<ApiResponse<Integer>> registRecipeFromRecommend(
			@PathVariable Integer rcdId,
			@AuthenticationPrincipal UserDetails userDetails) {

		Integer dishNo = recipeCommandService.registRecipeFromRecommend(rcdId, userDetails.getUsername());
