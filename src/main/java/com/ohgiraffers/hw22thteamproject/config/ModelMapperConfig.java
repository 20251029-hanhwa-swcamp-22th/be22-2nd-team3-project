...
public class ModelMapperConfig {
  ...
  private void configureRecommendRecipeMapping(ModelMapper modelMapper) {
    ...
    Converter<RecipeRecommendResponse.DishCategory, Integer> categoryConverter =
        context -> context.getSource() == null ? null : context.getSource().ordinal() + 1;

    // 2. Ingredients List -> String 변환 컨버터
    // 예: "당근(1.0개), 양파(0.5개)" 형태
    Converter<List<RecipeRecommendResponse.IngredientDetail>, String> ingredientsConverter =
    ...
    // 예: "고기 -> 버섯, 설탕 -> 올리고당" 형태
    Converter<List<RecipeRecommendResponse.Substitution>, String> substitutionConverter =
        context -> context.getSource() == null ? null : context.getSource().stream()
            .map(sub -> String.format("%s -> %s", sub.getOriginal(), sub.getReplacement()))
            .collect(Collectors.joining(", "));

    // 4. Cookery & Tips List -> String 변환 컨버터 (줄바꿈으로 구분)
    Converter<List<String>, String> listToStringConverter =
        context -> context.getSource() == null ? null : String.join("\n", context.getSource());
    ...
    modelMapper.createTypeMap(RecipeRecommendResponse.class, RecommendRecipe.class)
        .addMappings(mapper -> {
          ...
          mapper.using(categoryConverter)
              .map(RecipeRecommendResponse::getDishCategory, RecommendRecipe::setDishCategoryNo);
