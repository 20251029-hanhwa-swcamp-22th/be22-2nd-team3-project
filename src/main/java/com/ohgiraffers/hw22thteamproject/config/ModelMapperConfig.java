...
public class ModelMapperConfig {
    ...
    private void configureRecommendRecipeMapping(ModelMapper modelMapper) {
        ...
        Converter<List<RecommendRecipeResponse.IngredientDetail>, String> ingredientsConverter = context -> context
                .getSource() == null ? null
        ...
        modelMapper.createTypeMap(RecommendRecipeResponse.class, RecommendRecipe.class)
                .addMappings(mapper -> {
                    ...
                    mapper.using(categoryConverter)
                            .map(RecommendRecipeResponse::getDishCategoryEnum, RecommendRecipe::setDishCategoryNo);

                    // (2) 재료 리스트 -> 문자열
                    mapper.using(ingredientsConverter)
                            .map(RecommendRecipeResponse::getIngredients, RecommendRecipe::setRcdRecipeIngredients);

                    // (3) 대체 재료 리스트 -> 문자열
                    mapper.using(substitutionConverter)
                            .map(RecommendRecipeResponse::getSubstitutions, RecommendRecipe::setRcdRecipeSubstitutions);

                    // (4) 조리법 리스트 -> 문자열
                    mapper.using(listToStringConverter)
                            .map(RecommendRecipeResponse::getCookery, RecommendRecipe::setRcdRecipeCookery);

                    // (5) 팁 리스트 -> 문자열
                    mapper.using(listToStringConverter)
                            .map(RecommendRecipeResponse::getTips, RecommendRecipe::setRcdRecipeTips);

                    // (6) 요리이름 매핑
                    mapper.map(RecommendRecipeResponse::getDishName, RecommendRecipe::setRcdRecipeDishName);
